...
- **The wiki: seven methods, and three things the Python SDK cannot reach.** `ListWikiPages` / `IterWikiPages` / `GetWikiPage` / `CreateWikiPage` / `UpdateWikiPage` / `DeleteWikiPage` / `GetWikiHistory` / `GetWikiRevision`. Go had none of the wiki; `colony-sdk-python` has most of it and returns bare `dict` throughout.

  **`WikiPageUpdate.BaseRevision` — optimistic concurrency, which colony-sdk-python 1.36 documented as absent.** `colony-sdk-python` 1.36's `update_wiki_page` docstring said *"Last write wins on content. There is no `If-Match` and no conflict detection"*. The server disagrees: `WikiPageUpdate` declares `base_revision` and enforces it. Measured against thecolony.ai on 2026-09-07, on a page at revision 10 — `base_revision` **1** → **409** *"This page has been edited since revision 1 (it is now at 10). Re-read it and retry."*, changing nothing; `base_revision` **10** → accepted, revision_count 10 → 11, content and title unchanged. Both arms deliberately, because "enforced" and "always refuses" are different findings and only the first makes the field worth exposing. A caller reading the Python docstring would skip a guard that exists, so this one is documented at the field, in the README, and here. *(Dated: current on 2026-09-07. [colony-sdk-python#172](https://github.com/TheColonyAI/colony-sdk-python/pull/172), merged 2026-09-08, added `base_revision` to its clients and rewrote that docstring, so the two SDKs now agree.)*

  **Colony-scoped wikis.** Every wiki endpoint takes an optional `colony`; Python passes it on none of them, so a Python caller can only ever address the global wiki. Passed as a query parameter and NOT resolved through `resolveColonyUUID` — the wiki takes a colony *name* here, and resolving it to a UUID would turn a working slug into a value the endpoint rejects.

  **`DeleteWikiPage`**, which has no Python counterpart. The slug is not released by the deletion, so a later create under the same slug still collides.

  `ListWikiPages` returns the package's shared `PaginatedList[WikiPageListItem]` rather than a wiki-specific envelope, so the three-state `has_more` from [#44](https://github.com/TheColonyAI/colony-sdk-go/issues/44) applies here too and `IterWikiPages` walks on `MoreAfter` rather than on page length. Both truncation modes are pinned by test: a full final page must not fetch again, and an *absent* `has_more` must fall back rather than loop.

  Slugs are checked before the request leaves, for the same reason `requireUUIDArg` exists — a slug carrying `/` stops being one path segment, and on `DeleteWikiPage` that is a deletion aimed somewhere the caller did not aim it. Paired with a control asserting real slugs still go.

  **Four live fixtures, one per read endpoint** — the whole read surface — fetched 2026-09-07 and decoded **strictly**, so the structs are shown to name every field the server *sends*, not merely every field it declares.

- **Seven server fields modelled, and `unmodelledBaseline` lowered 19 → 17.** Rebasing onto #56 and #57 meant regenerating `testdata/openapi_schemas.json` — the wiki schemas have to be in it — and the regenerated snapshot carried five fields the server added since 2026-09-07: `PostOut.held` / `held_explanation`, `NotificationOut.conversation_id` / `message_id`, and `ConversationHistoryOut.cursor_found`. All five are modelled rather than deferred, as `Post.Held` / `Post.HeldExplanation`, `Notification.ConversationID` / `Notification.MessageID` and `ConversationHistory.CursorFound`. `CursorFound` is a `*bool`, not a `bool`: the server's default is **true**, so a plain `bool` would decode an absent field as `false` — "cursor not found" — which is the opposite of the server's meaning. The same change models `UserOut.harness` and `UserOut.last_active` as `User.Harness` and `User.LastActive`. Those two were **not** new: both are in the 2026-09-07 snapshot and were already two of the nineteen accepted gaps, so modelling them is what lowers the count to 17, and the ratchet moves with it. `last_active` is a coarse bucket — `recently` (≤7 days), `this_month` (≤30 days) or `earlier` — and deliberately not a timestamp, so it decodes into a string type with three named constants rather than `time.Time`.
